...
def ensure_job_kind_constraint(db):
    ...
    if row is None or "google_image_import" not in (row["sql"] or ""):
        return
    ...
    db.execute(
        ...
        WHERE kind IN ('convert_images', 'compact_images', 'vacuum_database')
        """
    )
...
register_chapter_routes(app, sys.modules[__name__])
