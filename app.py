...
def search_timeline_content(db, query):
    ...
    photo_rows = db.execute(
        ...
        LIMIT 40
        """,
        (g.user["id"], pattern, pattern, pattern, pattern, pattern, pattern, pattern),
    ).fetchall()
    for row in photo_rows:
        ...
        WHERE user_id = ?
          AND (
            ...
            OR EXISTS (
                ...
                WHERE tep.entry_id = text_entries.id
                  AND pe.user_id = text_entries.user_id
                  AND lower(pe.name) LIKE ?
            )
          )
